...
function App() {
  ...
  const [showFicha, setShowFicha] = useState(false); // Controla si se muestra la ficha
  ...
  const findPeopleAxios = useCallback(async () => {
    if (isLoading) return;
    setIsLoading(true);
    setShowFicha(false); // Oculta la ficha cuando se busca
    setLoadingType('axios');
    setPeople({ axios: [], fetch: [] });
    setSelectedUser(null); // Reinicia la selección
    try {
  ...
  // Se encarga de actualizar el usuario seleccionado (único)
  const handleSelectUser = (user) => {
    setSelectedUser(user);
  };

  // Muestra la ficha solo si existe un usuario seleccionado
  const mostrarFicha = () => {
    if (selectedUser) {
      setShowFicha(true);
    } else {
  ...
  return (
    <div className="App">
      ...
      {showFicha ? (
        <FichaPersonal user={selectedUser} />
      ) : (
        <>
                ...
                {people.axios.length > 0 ? (
                  people.axios.map(person => (
                    <Person
                      ...
                      isSelected={selectedUser && selectedUser.login.uuid === person.login.uuid}
                    />
                  ))
                ) : (
